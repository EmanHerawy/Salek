...
contract CrossChainHook is BaseHook {
   ...
   if (hookData.length == 0) return (this.afterSwap.selector, 0);
    ...
    uint256 amountToSwap =  swapParams.zeroForOne ?  uint256(int256(-delta.amount0())): uint256(int256(- delta.amount1()));
     _swapCrossChain(tokenToSwap, receiver,swapParams.zeroForOne? address(0): linkToken, amountToSwap, chainSelector);
...
// TODO: we need a way to reduce the balance after cross chain swapping , otherwise the user can swap the same token multiple times and drain the pool
    function _swapCrossChain(address tokenAddress, address receiver , address feeTokenAddress,uint256 amount,  uint64 destinationChainSelector) internal {
